...
* clicking on the plane. Kernel should be run with dimensions equal to the desired
* brush and with a global offset to where the bottom right corner should be.
* color should be an array of 4 floats from [0-1] and in RGBA order
*/
__kernel void set_pixel(__global float *color, write_only image2d_t img,
	read_only image2d_t img_read)
{
	int2 img_dim = get_image_dim(img);
	int2 coord = (int2)(get_global_id(0), get_global_id(1));
	//Because of some wierdness on Nvidia we must read the color data, then modify it
	//otherwise we just output white for some reason. I'm looking into this issue separately
	//could be a bug of some sort
	...
	px_color = px_color * 0.f + (float4)(color[0], color[1], color[2], color[3]);
	write_imagef(img, coord, px_color);
}
/*
* Allows us to interact with the fluid by applying forces to cells
* The kernel should be run with a global offset to where the bottom-right corner of the brush
